...
import org.entirej.applicationframework.tmt.renderers.item.definition.interfaces.EJTMTLabelItemRendererDefinitionProperties;
...
public class EJTMTLabelItemRenderer implements EJTMTAppItemRenderer, FocusListener, Serializable
{
    ...
    public void initialise(EJScreenItemController item, EJScreenItemProperties screenItemProperties)
    {
        ...
        final String caseProperty = _rendererProps.getStringProperty(EJTMTLabelItemRendererDefinitionProperties.PROPERTY_CASE);
        if (caseProperty != null && caseProperty.trim().length() > 0)
        {
            if (caseProperty.equals(EJTMTLabelItemRendererDefinitionProperties.PROPERTY_CASE_LOWER))
            {
                _valueCase = VALUE_CASE.LOWER;
            }
            else if (caseProperty.equals(EJTMTLabelItemRendererDefinitionProperties.PROPERTY_CASE_UPPER))
            {
    ...
    {
        _labelField.getControl().setFont(EJTMTVisualAttributeUtils.INSTANCE.getFont(_visualAttributeProperties, _visualContext.getItemFont()));
    }
    ...
    {
        String alignmentProperty = _rendererProps.getStringProperty(EJTMTLabelItemRendererDefinitionProperties.PROPERTY_ALIGNMENT);
        if (alignmentProperty == null)
        {
            alignmentProperty = _rendererProps.getStringProperty("ALLIGNMENT");
        }
        boolean textWrapProperty = _rendererProps.getBooleanProperty(EJTMTLabelItemRendererDefinitionProperties.PROPERTY_TEXT_WRAP, false);
        ...
        _displayAsHyperlink = _rendererProps.getBooleanProperty(EJTMTLabelItemRendererDefinitionProperties.PROPERTY_DISPLAY_AS_HYPERLINK, false);
        ...
        if (!_displayAsHyperlink)
        {
            ...
            if (pictureName != null && pictureName.length() > 0)
            {
                ...
                {
                    labelField.setImage(EJTMTImageRetriever.get(pictureName));
                }
            }
        ...
        _visualContext = new EJTMTItemRendererVisualContext(_labelField.getControl().getBackground(), _labelField.getControl().getForeground(), _labelField
                .getControl().getFont());
    ...
    protected int getComponentStyle(String alignmentProperty, int style)
    {
        ...
        {
            if (alignmentProperty.equals(EJTMTLabelItemRendererDefinitionProperties.PROPERTY_ALIGNMENT_LEFT))
            {
                style = style | SWT.LEFT;
            }
            else if (alignmentProperty.equals(EJTMTLabelItemRendererDefinitionProperties.PROPERTY_ALIGNMENT_RIGHT))
            {
                style = style | SWT.RIGHT;
            }
            else if (alignmentProperty.equals(EJTMTLabelItemRendererDefinitionProperties.PROPERTY_ALIGNMENT_CENTER))
            {
    ...
    public Cell<? extends Cell<?>> createColumnCell(EJScreenItemProperties item, EJScreenItemController controller, Template template)
    {
        ...
        textCell.setWrap(_rendererProps.getBooleanProperty(EJTMTLabelItemRendererDefinitionProperties.PROPERTY_TEXT_WRAP, false));
        return textCell;
    }
}
