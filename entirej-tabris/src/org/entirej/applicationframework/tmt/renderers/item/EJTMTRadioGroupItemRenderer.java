...
public class EJTMTRadioGroupItemRenderer implements EJTMTAppItemRenderer, FocusListener, Serializable
{
    private Map<String, RadioButtonValue>   _radioButtons;
    private Composite                       _radioGroup;
    private boolean                         _displayFrame         = true;
    private EJFrameworkExtensionProperties  _rendererProps;
    private EJScreenItemController          _item;
    private EJScreenItemProperties          _screenItemProperties;
    private EJItemProperties                _itemProperties;
    ...
    private EJCoreVisualAttributeProperties _initialVAProperties;
    ...
    public void initialise(EJScreenItemController item, EJScreenItemProperties screenItemProperties)
    {
        ...
        _displayFrame = _rendererProps.getBooleanProperty(EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_SHOW_BORDER, true);
    ...
    private void refreshFont(Button radioButton)
    {
        if (radioButton != null)
        {
            radioButton.setFont(EJTMTVisualAttributeUtils.INSTANCE.getFont(_visualAttributeProperties, _visualContext.getItemFont()));
        }
    }
    ...
    public void createComponent(Composite composite)
    {
        ...
        if (EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_ORIENTATION_VERTICAL.equals(_rendererProps
                .getStringProperty(EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_ORIENTATION)))
        {
            type = SWT.VERTICAL;
        }
        ...
        EJFrameworkExtensionPropertyList radioButtons = _rendererProps.getPropertyList(EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_RADIO_BUTTONS);
        for (EJFrameworkExtensionPropertyListEntry listEntry : radioButtons.getAllListEntries())
        {
            Object value = getValueAsObject(_item.getReferencedItemProperties().getDataTypeClass(),
                    listEntry.getProperty(EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_VALUE));
            Button button = new Button(_radioGroup, SWT.RADIO);

            // Store the button and the button values for future reference
            _radioButtons.put(listEntry.getProperty(EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_NAME), new RadioButtonValue(button, value));

            // Set the button properties
            button.setData(listEntry.getProperty(EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_NAME));
            button.setText(_item.getForm().translateText(listEntry.getProperty(EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_LABEL)));

            // button.setActionCommand(screenItemProperties.getActionCommand());

            if (_rendererProps.getStringProperty(EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_DEFAULT_BUTTON) != null)
            {
                if (_rendererProps.getStringProperty(EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_DEFAULT_BUTTON).equals(
                        listEntry.getProperty(EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_NAME)))
                {
    ...
    protected class RadioButtonValue
    {
        ...
        private EJTMTRadioGroupItemRenderer getOuterType()
        {
            return EJTMTRadioGroupItemRenderer.this;
        }
    }
    ...
    public ColumnLabelProvider createColumnLabelProvider(final EJScreenItemProperties item, EJScreenItemController controller)
    {
        ...
        for (EJFrameworkExtensionPropertyListEntry listEntry : radioButtons.getAllListEntries())
        {
            Object value = getValueAsObject(_item.getReferencedItemProperties().getDataTypeClass(),
                    listEntry.getProperty(EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_VALUE));
            String lable = _item.getForm().translateText(listEntry.getProperty(EJTMTRadioButtonItemRendererDefinitionProperties.PROPERTY_LABEL));
