 ...
 * "Form Package" in applicaiton.ejprop file.<br>
 * Each package will be a menu and each file will be a menu item.<br>
 */
public class EJTMTDefaultMenuPropertiesBuilder
{
    private EJTMTDefaultMenuPropertiesBuilder()
    {
    }
     ...
     * 
     * @return a menu tree root object for opening the forms from the defined
     *         packages
     */
    public static EJTMTMenuTreeRoot buildMenuProperties(EJApplicationManager applicationManager, String menuId)
    {
        EJTMTMenuTreeRoot root = new EJTMTMenuTreeRoot();
    ...
    private static void addEJCoreMenuLeafProperties(EJTMTMenuTreeElement parent, EJCoreMenuLeafProperties leafProperties)
    {

        if (leafProperties instanceof EJCoreMenuLeafContainer)
        {
            EJTMTMenuTreeElement subMenu = parent.addSubTreeElement(leafProperties.getDisplayName(), null, leafProperties.getIconName());
